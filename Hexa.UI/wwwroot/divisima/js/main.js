...
function addCart(productid, stock) {
	...
	if (istenenMiktar <= stock) {
		$.ajax({
			...
			success: function (data) {
				if (data.indexOf("~") == -1) {
					//$('#modalSepet .modal-body').text(data + " ürününden " + istenenMiktar + " adet sepete baþarýyla eklendi...")
					//$('#modalSepet').modal('show')
					alert("urunden "+istenenMiktar+" adet sepete eklendi")
					getCartCount();
				}
			},
			error: function (e) {
		...
		alert("Bu üründen en fazla " + stock + " adet sipariþ verebilirsiniz")
	}
}
